      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              >
                ...
                {(['base', 'background', 'foreground'] as const).map(type => {
                  ...
                  return (
                    <div key={type} className="mb-2">
                      ...
                      {/* {showImagePreview?.[page.id]?.[type] && page[`${type}Image`] && (
                        <div className="mt-2">
                          <img
                            src={page[`${type}Image`]!}
                            alt={`${type} preview`}
                ...
                {/* {index === currentPageIndex && page.textBoxes.map((textBox) => (
                  <div key={textBox.id} className="flex gap-1 mt-2 items-center">
                    <button
                      className={`flex-1 text-white p-2 rounded-md text-sm transition-colors ${
                        selectedTextBox?.id === textBox.id ? 'bg-blue-600' : 'bg-red-400 hover:bg-red-500'
                      }`}
                      onClick={() => setSelectedTextBox(textBox)}
                    >
                      Text Box
                ...
                {/* {index === currentPageIndex && page.textBoxes.map((textBox) => (
                  <button 
                    key={textBox.id} 
                    className={`w-full mt-2 text-white p-2 rounded-md text-sm transition-colors ${
                      selectedTextBox?.id === textBox.id ? 'bg-blue-600' : 'bg-red-400 hover:bg-red-500'
                    }`}
                    onClick={() => setSelectedTextBox(textBox)}
                  >
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 bg-blue-50 p-6 overflow-auto">
            ...
            {/* <div className="aspect-[877/620] w-auto max-h-[90%] mx-auto shadow-lg"> */}
            {/* <div ref={canvasRef} className="w-full h-full relative"> */}
              {/* <Canvas 
                ...
                initialTextBoxes={currentPage.textBoxes}
                onTextBoxesChange={saveCurrentPageTextBoxes}
          ...
          {/* <div className="flex justify-end p-2 bg-white border-t border-gray-200">
            <Button variant="outline" size="sm" className="mr-1">Up</Button>
            <Button variant="outline" size="sm" className="mr-1">Down</Button>
